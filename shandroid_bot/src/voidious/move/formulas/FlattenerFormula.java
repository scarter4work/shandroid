...
public class FlattenerFormula extends DistanceFormula
{
	...
	public double[] dataPointFromWave(Wave w, boolean aiming)
	{
		return new double[] { (Math.min(91, w.getTargetDistance() / w.getBulletSpeed()) / 91),
				...
				(Math.min(1.0, w.getTargetWallDistance()) / 1.0), (Math.min(1.0, w.getTargetRevWallDistance()) / 1.0),
				(Math.min(1, (w.getTargetVchangeTime()) / (w.getTargetDistance() / w.getBulletSpeed()))),
				(w.getTargetDl8t() / 64), (w.getTargetDl20t() / 160), (w.getTargetDl40t() / 320), };
	}
}
