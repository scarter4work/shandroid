...
public class BattleField
{
	...
	{
		this.rectangle = new Rectangle2D.Double(18, 18, width - 36, height - 36);
		this.width = width;
		this.height = height;
	}
	...
	{
		return new Point2D.Double(DiaUtils.limit(18, p.x, this.width - 18), DiaUtils.limit(18, p.y, this.height - 18));
	}

	public double orbitalWallDistance(Point2D.Double sourceLocation, Point2D.Double targetLocation, double bulletPower,
			int direction)
	{
		...
		for (int x = 0; x < 200; x++)
		{
			if (!this.rectangle.contains(sourceLocation.x
					+ (Math.sin(absBearing + (direction * (x / 100.0) * maxEscapeAngle)) * distance),
	...
	double directToWallDistance(Point2D.Double targetLocation, double distance, double heading, double bulletPower)
	{
		...
		{
			if (!this.rectangle.contains(targetLocation.x + (sinHeading * 8.0 * x), targetLocation.y + (cosHeading * 8.0 * x)))
			{
	...
	{
		double wallDistanceX = Math.min(startLocation.x - 18, this.width - startLocation.x - 18);
		double wallDistanceY = Math.min(startLocation.y - 18, this.height - startLocation.y - 18);
		...
		double testDistanceX = Math.min(testX - 18, this.width - testX - 18);
		double testDistanceY = Math.min(testY - 18, this.height - testY - 18);
		...
		while ((testDistanceX < 0 || testDistanceY < 0) && g++ < 25)
		{
			...
			if (adjacent < 0)
			{
				...
				angle += Math.PI - orientation * (Math.abs(Math.acos(-adjacent / wallStick)) - 0.0005);
			}
			else
			{
				angle += orientation * (Math.abs(Math.acos(adjacent / wallStick)) + 0.0005);
			}
			testX = startLocation.x + (Math.sin(angle) * wallStick);
			testY = startLocation.y + (Math.cos(angle) * wallStick);
			testDistanceX = Math.min(testX - 18, this.width - testX - 18);
			testDistanceY = Math.min(testY - 18, this.height - testY - 18);
		}
